...
## 2026-07-13 — Session: post-Living-World loose ends (branch `claude/codebase-review-agents-nts5xt`)

### Cleanup batch — complete; suite 253 passed / 2 skipped (+1 flaky, passes isolated); `npm run build` clean
- **Added** `/world` IMPACT ENGINES panel — `WorldCommand.jsx` now renders the streamed `engines` message (was received + dropped): "⬡ ENGINES" tab above the BRIEF tab, `wc-panel` listing the 7 domains with confidence %, up to 3 scalar metrics, first recommendation. New `wc-engines-tab`/`wc-engine*` styles in `WorldCommand.css`.
- **Fixed** `FlyoverSim.jsx:14` — `API_BASE` now `import.meta.env.VITE_API_BASE || 'http://localhost:8001'` (matches Flyover3DViewer/ImagenFlyoverVisuals; `/plan-coords` still lives in the separate `ai-flyover-sim/backend`, so the env override is the seam, not a port of that endpoint).
- **Fixed** model-name drift — all stale Llama 3.3 70B references → Kimi k2.6 (the model `llm/chat.py` actually calls for the "analysis" slot): `llm/config.py` header, `llm/routing.py` (LLAMA profile renamed KIMI, name `moonshotai/kimi-k2.6:free`), `llm/chat.py` (`ensemble` docstring + availability check accepted dead key "llama", now "kimi"), `app.py` `/ldrago/status` reasoner row, `agents/cognitive/{orchestrator,reasoner_agent,synthesizer_agent,planner_agent}.py`, `agents/ldrago_orchestrator.py`, `.env.example` (`LLAMA_MODEL`→`KIMI_MODEL`), `ARCHITECTURE.md` tables/diagrams.
- **Docs honest** — `/live/route` + `fetch_demo_route_geojson` docstrings said "currently returns a demo stub"; the endpoint tries real OSRM first, so docstrings now describe the labeled (`"source": "demo_stub"`) fallback path.
- **Env notes**: bare container needed `pip install` of the light dep set (no torch/sentence-transformers required by the suite) and the known `npm install --no-save @rollup/rollup-linux-x64-gnu @esbuild/linux-x64` Linux-natives fix before `npm run build`.
- **Known flake (pre-existing)**: `tests/world/test_session.py::test_create_session_registers_and_streams_frames` can fail under the full parallel suite load; passes standalone and in `tests/world/` runs.
